...
function getErrorTip({ error, errorTip }) {
  ...
  if (error?.message?.startsWith('timeout of')) return '访问资源超时'

  return error?.message || error?.data?.msg || serverErrorTip
}
...
export default function handleError({ error, errorTip }) {
  ...
  // if (status === 401) return console.log('支登录')

  // 用户设置false，关闭提示，不显示错误信息
  if (errorTip === false) return

  const errorMessage = getErrorTip({ error, errorTip })
  if (errorMessage) {
